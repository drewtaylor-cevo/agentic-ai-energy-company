...
- **D-01 (REVISED 2026-04-26 post-research):** CFN stack policies applied **via `aws cloudformation set-stack-policy` CLI as a ceremony step in plan 10-03** — NOT CDK-native. Policy bodies live as committed assets under `infrastructure/stack-policies/{foundation,agentcore,backend-api}-freeze.json`. Ceremony command per stack: `aws cloudformation set-stack-policy --stack-name <name> --stack-policy-body file://infrastructure/stack-policies/<name>-freeze.json --profile cevo-dev25`. Rationale: researcher (10-RESEARCH.md §Q1) empirically verified that `stack.node.default_child.add_override("StackPolicy", ...)` does NOT work on root Stacks — root stacks have `node.default_child == None`; CFN Stack Policies are a separate out-of-band API, not part of the template body. Rejected alternative AwsCustomResource-in-each-stack (CDK-native) because it adds a custom-resource Lambda + Role + LogGroup per stack, polluting `cdk diff`, and risks self-modifying-stack loops. Break-glass relock command mirrors the forward command with the allow-all policy body. Trade-off: a future `cdk deploy` does NOT re-apply the policy (CDK isn't aware of it); freeze-ceremony-only application is the contract. Matches D-03's "freeze-time posture via CLI, not permanent CDK code" framing — stack policy joins termination protection as a CLI-applied freeze posture. Break-glass + relock commands go into FREEZE-MANIFEST.md (D-04).
...
- **D-08 (REVISED 2026-04-26 post-research):** Lambda bundle reproducibility proved by **`cdk synth` twice + sha256 of a per-asset content manifest** (NOT the raw zip). Researcher (10-RESEARCH.md §Q3) flagged that CDK Docker bundling writes `.pyc` files into `cdk.out/asset.<hash>/` with current-build timestamps, so zip-file hashing is non-deterministic even when bundle content is identical. Replacement pattern: for each asset dir under `cdk.out/`, produce `(cd <asset_dir> && find . -type f -not -name '*.pyc' -not -path '*/__pycache__/*' -print0 | sort -z | xargs -0 sha256sum | sha256sum)` — strips `.pyc`/`__pycache__`, sorts, hashes content only. Sequence: (1) clean git clone at freeze commit, fresh venv, `pip install --require-hashes -r requirements.txt`, `cdk synth`, record per-asset content-hash; (2) separate tmp dir, repeat; (3) assert identical. Manifest stores: freeze commit SHA + per-asset content-hash (keyed by asset logical ID from `cdk.out/manifest.json`, not by asset.<hash> dir name which is itself content-derived). The synth-twice evidence is the *output* proof that matters for the Lambda bundle at deploy time. Wheel hashes in `requirements.txt` are the *input* proof, not duplicated.

- **D-09 (REVISED 2026-04-26 post-research):** UI dist hashing: **sha256 of a deterministic content manifest per dist dir** — NOT raw tar. Researcher (10-RESEARCH.md §Q4) empirically proved that `find ui/dist -type f | sort | tar -cf - -T - | sha256sum` leaks file mtimes into tar headers, so a rebuild of byte-identical content produces a different hash (observed `940a40e7…` vs `681134577…`). This would guarantee drill D-16 hash-roundtrip failure. Replacement pattern: `(cd ui/dist && find . -type f -print0 | sort -z | xargs -0 sha256sum | sha256sum)` — sorted per-file sha256 digest then hash-of-hashes. No tar, no mtime, presenter-readable (the intermediate sha256sum output can be shown as evidence). Same pattern for `ui/dist-mock/`. Two sha256s total in the manifest. Planner verifies cross-rebuild stability empirically in plan 10-02 (rebuild, rehash, assert identical) before the pattern is blessed. Fallback if any instability: commit a content manifest file (`ui/dist.sha256` listing `path<TAB>hash` sorted) and hash that file. Rejected alternatives preserved from original D-09: per-file hash listing in manifest body (too noisy), gzipped archive hash (non-portable). Freeze ceremony rebuilds both dists from source before hashing.
...
- **D-18 (REVISED 2026-04-26):** T-48h ceremony step order (load-bearing dependency chain):
  ...
  4. **Stack lock** — `aws cloudformation set-stack-policy` per stack using `infrastructure/stack-policies/<name>-freeze.json` (D-01 revised); manually enable termination protection on all three stacks via CLI (D-03). No `cdk deploy` in ceremony — stack policy is CLI-applied, not template-bound.
  5. **DynamoDB backup** — `aws dynamodb create-backup --table-name tariff-billing --backup-name tariff-billing-freeze-v2.0-<UTC-date>`. Wait-loop until `BackupStatus == AVAILABLE` before capturing ARN + timestamp.
  6. **FREEZE-MANIFEST.md** — compute all hashes (D-08, D-09, D-10), fill manifest, commit.
  7. **Tag** — `git tag -a demo-v2.0 -m "..." <freeze-commit-SHA>` on the manifest-commit SHA; **`git push origin demo-v2.0`** (revised 2026-04-26 — origin IS configured, `demo-v1.0` was already pushed, peer/CI verifiability preserved).

- **D-19 (REVISED 2026-04-26):** Reproducibility gate operator: **freeze owner in a fresh git clone + fresh venv**. Commands: `git clone /path/to/repo /tmp/freeze-repro && cd /tmp/freeze-repro && python -m venv .venv && .venv/bin/pip install --require-hashes -r requirements-dev.txt && .venv/bin/pytest -m "not smoke"`. Captured stdout tail (showing pass count) goes into FREEZE-MANIFEST.md evidence. Note: origin IS configured and `demo-v1.0` / `demo-v2.0` ARE pushed — peer-machine or CI verification becomes feasible and is marked as a deferred opportunity rather than rejected-as-impossible. Single-operator fresh-clone discipline remains the Phase 10 baseline; peer/CI is a v3.0 upgrade path.
...
- **D-21 (REVISED 2026-04-26):** Phase 10 splits into **3 plans**:
  - **10-01** (Wave 1, autonomous): Stack-policy + reproducibility-helper artefacts — (a) commit deny-Update:* stack policy JSON bodies under `infrastructure/stack-policies/{foundation,agentcore,backend-api}-freeze.json` plus allow-all break-glass bodies; (b) commit helper scripts that compute the content-manifest hashes for D-08 (synth assets) and D-09 (UI dist) — inline under `scripts/` following Phase 9 exit-code taxonomy. Empirically verify on the planner's machine that a rebuild of `ui/dist/` produces an identical content-manifest hash (cross-rebuild determinism gate from D-09 revised). No AWS changes. No CDK code changes — D-01 revised moves policy application to ceremony-time CLI, not stack code.
  - **10-02** (Wave 2, autonomous, depends on 10-01): Freeze ceremony artefacts — run `pip-compile --generate-hashes` to produce hash-pinned `requirements.txt` + `requirements-dev.txt` (create `requirements.in` + `requirements-dev.in` source files first); scaffold FREEZE-MANIFEST.md template (empty hash fields, all structural keys from D-07/D-10 present, break-glass block populated with revised D-01 set-stack-policy commands); amend DEMO-RUNBOOK.md with §3–§6 per D-20. This plan does NOT fill the manifest hashes or cut the tag — those belong to 10-03.
  - **10-03** (Wave 3, **autonomous: false** — human checkpoint before tag): T-48h ceremony execution — operator runs steps 1–7 of the revised D-18 sequence, fills FREEZE-MANIFEST.md hashes, runs the rollback drill (populates 10-DRILL-LOG.md), takes DynamoDB backup, applies stack policies via CLI, enables termination protection, cuts `demo-v2.0` tag, and pushes to origin per revised D-18 step 7. Human checkpoint: 'drill passed, manifest complete, safe to tag?' before the `git tag` step. Second checkpoint (lower-stakes): 'push demo-v2.0 to origin now?' before `git push origin demo-v2.0`.
