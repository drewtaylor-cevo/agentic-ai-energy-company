...
| 10-03-08 | 03 | 3 | DEMO-04 | D-19 reproducibility gate: fresh clone + fresh venv pytest green (Rule 3 codified per Rule 4 remediation 2026-04-26: `/opt/homebrew/bin/python3.13` — `/usr/bin/python3` is 3.9.6 and cannot install iniconfig==2.3.0 wheel; install BOTH prod + dev lockfiles since `-c` constraint layering means dev does not re-emit prod hashes but pytest collection hits agent/agent.py which imports pydantic+strands+bedrock_agentcore from prod; `export AWS_PROFILE=cevo-dev25` prevents ProfileNotFound on backend-api-handler test collection) | reproducibility | `git clone . /tmp/freeze-repro && cd /tmp/freeze-repro && /opt/homebrew/bin/python3.13 -m venv .venv && .venv/bin/pip install --require-hashes -r requirements.txt -r requirements-dev.txt && export AWS_PROFILE=cevo-dev25 && .venv/bin/pytest -m "not smoke"` → `189 passed, 34 deselected` (Phase 10 ceremony baseline with AWS creds present — strictly better than Phase 9 closeout 183/6/34; the 6 previously-skipped slots are AWS-dependent tests that now pass) | ❌ W0 | ⬜ pending |
...
| 10-03-13 | 03 | 3 | DEMO-06 | `build:mock` intra-HEAD build determinism (softened per Rule 4 D-16 decision 2026-04-26 — cross-HEAD reproducibility NOT gated here; vite.config.ts embeds __GIT_SHA__ so dist_bundles.* hashes are commit-bound snapshots. Cross-HEAD reproducibility is guarded by lockfile hashes + synth_asset hashes + `cdk diff == 0`.) | equality | `rm -rf ui/dist-mock && npm --prefix ui run build:mock >/dev/null 2>&1 && H1=$(scripts/hash_dist.sh ui/dist-mock \| awk '{print $1}') && rm -rf ui/dist-mock && npm --prefix ui run build:mock >/dev/null 2>&1 && H2=$(scripts/hash_dist.sh ui/dist-mock \| awk '{print $1}') && [ "$H1" = "$H2" ]` | ❌ W0 | ⬜ pending |
