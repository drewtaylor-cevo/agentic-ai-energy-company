...
| 10-02-06 | 02 | 2 | DEMO-04 | DEMO-RUNBOOK.md has new §3–§6 headings | artefact | `grep -cE '^## §[3-6]' .planning/milestones/v1.0-phases/05-demo-hardening/DEMO-RUNBOOK.md` returns `4` | ❌ W0 | ⬜ pending |
...
| 10-03-08 | 03 | 3 | DEMO-04 | D-19 reproducibility gate: fresh clone + fresh venv pytest green | reproducibility | `git clone . /tmp/freeze-repro && cd /tmp/freeze-repro && python3 -m venv .venv && .venv/bin/pip install --require-hashes -r requirements-dev.txt && .venv/bin/pytest -m "not smoke"` → `81 passed, 6 skipped` | ❌ W0 | ⬜ pending |
