...
---
*Last updated: 2026-04-26 — Phase 9 complete (DEMO-03 complete, DEMO-05). `scripts/prewarm.py` + `npm run prewarm` wrapper ship the two-pass warm + measurement CLI with 3000ms median gate and 0/1/2 exit taxonomy; `scripts/demo-keepalive.sh` rotates CUST-001→CUST-002→CUST-003 every 10 minutes with trap-on-INT/TERM/HUP clean shutdown; `tests/test_narrative_eval_live.py` (smoke-gated) asserts Phase 6 validator rules + Phase 7 `_narrative_source` stripping against the live stack; offline `tests/test_prewarm_script.py` covers all 7 D-20 cases. 3 live-stack D-22 gates deferred to operator closeout. Next: Phase 10 Freeze + Rollback Drill.*
