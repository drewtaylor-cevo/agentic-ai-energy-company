...
- [x] Solar PV + EV personas seeded with realistic billing profiles — v3.0 Phase 11 (CUST-004 solar, CUST-005 EV) (DATA-04, DATA-05)
- [x] Hardship persona with `hardship_flag` PROFILE row — v3.0 Phase 11 (CUST-006) (DATA-06)
- [x] Byte-exact savings locked for new personas — v3.0 Phase 11 (CUST-004 ECO $40.02 / SOL $76.03, CUST-005 ECO $35.00 / EV-TOU $84.00, CUST-006 ECO $12.00 / VAL $22.00) (DATA-07)
- [x] Solar Feed-in tariff archetype — v3.0 Phase 11 (`SOL` with `plan_type: solar_fit`, dispatcher branch in `simulate_savings_pure`) (REC-04)
- [x] EV Time-of-Use tariff archetype — v3.0 Phase 11 (`EV-TOU` with `plan_type: time_of_use`, dispatcher branch in `simulate_savings_pure`) (REC-05)
- [x] `CustomerDataProvider` abstraction — production-shaped `@runtime_checkable` Protocol (3 methods: `get_customer` / `get_billing_history` / `get_hardship_flag`) with `ToolsLambdaProvider` (production boto3 invoke), `InMemoryProvider` (offline test double, byte-exact savings), and `SalesforceCustomerDataProvider` (DOC-03 presenter stub raising `NotImplementedError`); lambda/handler.py gained a top-level `handler(event, context)` action dispatcher; SAV-03 invariant preserved byte-equal through the strangler-fig indirection (40/40 pre/post live-diff fields byte-equal across CUST-001..005); ROADMAP SC #5 bi-mode container+repo import dual-gate green — v3.0 Phase 12 (PROD-01, PROD-01a, PROD-01b, PROD-01c)
...
---
*Last updated: 2026-04-29 — Phase 12 complete (PROD-01, PROD-01a, PROD-01b, PROD-01c validated). `agent/providers.py` strangler-fig seam: `@runtime_checkable CustomerDataProvider` Protocol with `ToolsLambdaProvider` (production) + `InMemoryProvider` (offline test double) + `SalesforceCustomerDataProvider` (DOC-03 presenter stub). `lambda/handler.py` gained action dispatcher routing on `event["action"]` (back-compat default = `simulate_savings` per D-05). Stack-policy lift → deploy (CustomerTariff + CustomerTariffAgent) → `--mode compare` byte-equality gate (40/40 numeric fields byte-equal across 5 personas) → re-freeze ceremony (~31min window) executed 2026-04-29 00:17Z–00:24Z. ROADMAP SC #5 bi-mode container+repo dual-gate green (`from providers import CustomerDataProvider` in AgentCore container AND `from agent.providers import CustomerDataProvider` in repo venv). End-to-end public API smoke green for CUST-001 byte-exact + CUST-006 (Phase 14 hardship short-circuit correctly NOT yet shipped). Deny·Deny·Deny freeze restored byte-equal across CustomerTariff + CustomerTariffAgent + CustomerTariffApi; CustomerTariffApi untouched (D-07). Code review: 0 critical / 5 warning / 7 info (non-blocking). Milestone v2.0 feature-complete (Phase 10 freeze at `demo-v2.0` tag still authoritative); v3.0 milestone execution in progress.*
